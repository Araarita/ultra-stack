...
def request_approval(action, description, params=None):
    """Crea solicitud de approval y notifica con BOTONES INLINE."""
    ...
    notify_approval_with_buttons(request_id, action, description)
...
def notify_approval_with_buttons(request_id, action, description):
    """Envia approval con botones inline de Telegram."""
    if not BOT_TOKEN or not OWNER_ID:
        return
    
    brief = description[:600]
    if len(description) > 600:
        brief += "..."
    
    msg = "APPROVAL NECESARIA\n\n"
    msg += "Accion: " + action + "\n\n"
    msg += "Detalle:\n" + brief + "\n\n"
    msg += "ID: " + request_id + "\n"
    msg += "Expira en 10 min"
    
    keyboard = {
        "inline_keyboard": [
            [
                {"text": "Aprobar", "callback_data": "approve:" + request_id},
                {"text": "Rechazar", "callback_data": "reject:" + request_id},
            ],
            [
                {"text": "Ver completo", "callback_data": "detail:" + request_id},
                {"text": "Modificar", "callback_data": "modify:" + request_id},
            ],
            [
                {"text": "Posponer 1h", "callback_data": "snooze:" + request_id},
            ]
        ]
    }
